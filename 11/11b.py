...
class gameoflife:
    ...
    def iterate_once(self):
        ...
        for (y, x), s in np.ndenumerate(self.state):
            ...
            seat_counts = self.free_vector(y, x)
            if seat_counts[False] == 0:
                new_state_matrix[y, x] = 2
            elif seat_counts[False] >= 5:
    ...
    def free_vector(self, y, x):
        ...
        for d in directions:
            ...
            while not found_seat:
                # check bounds here is + 1 since we padded
                if (y + j <= 0) or (x + i <= 0):
                    seats_free.append(True)
                    break
                if (y + j >= self.xbound + 1) or (x + i >= self.xbound + 1):
                    seats_free.append(True)
                    break

                if self.state[y + j, x + i] == 1:
                    found_seat = True
                    seats_free.append(True)
                elif self.state[y + j, x + i] == 2:
                    found_seat = True
                    seats_free.append(False)
                else:
                    # Look further
